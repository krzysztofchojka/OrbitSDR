...
#include "UI.h"
#include "Sidebar.h" // Używa Twojego nowego pliku sidebar.h
...
struct SharedData { 
    ...
    float squelchThreshold = -100.0f; bool stereoEnabled = false; Mode mode = Mode::NFM; bool isPlaying = false; float minDb = -120.0f; float maxDb = 0.0f; std::vector<double> fftSpectrum; std::vector<uint8_t> waterfallRow; bool newWaterfallData = false; std::string currentFilename = "None"; float mouseX_spectrum = -1.0f; float mouseY_spectrum = -1.0f; bool isRecording = false; RecMode recMode = RecMode::AUDIO; std::string recPath = ""; std::string recStatus = "Idle"; bool aprsEnabled = false; std::deque<std::string> aprsLog; AprsLastPacket lastAprs; SharedData() : fftSpectrum(FFT_SIZE, -100.0), waterfallRow(INTERNAL_WATERFALL_WIDTH * 4, 0) {} 
};

std::mutex sourceMtx; std::shared_ptr<IQSource> currentSource;

sf::Color getHeatmap(float v) { v = std::clamp(v, 0.0f, 1.0f); std::uint8_t r=0,g=0,b=0; if(v<0.25f) b=static_cast<std::uint8_t>(v*4*255); else if(v<0.5f) {b=255; g=static_cast<std::uint8_t>((v-0.25f)*4*255);} else if(v<0.75f) {r=static_cast<std::uint8_t>((v-0.5f)*4*255); g=255; b=static_cast<std::uint8_t>(255-r);} else {r=255; g=static_cast<std::uint8_t>((1.0f-v)*4*255);} return {r,g,b}; }
...
void drawGrid(sf::RenderWindow& window, const sf::Font& font, float x, float y, float w, float h, long long cf, int sr, float minDb, float maxDb) {
    float dbStep = 20.0f; for (float db = 0; db >= -140; db -= dbStep) { if (db > maxDb || db < minDb) continue; float norm = (db - minDb) / (maxDb - minDb); float yPos = y + h - (norm * h); sf::RectangleShape line({w, 1}); line.setPosition({x, yPos}); line.setFillColor(sf::Color(100, 100, 100, 150)); window.draw(line); sf::Text l(font, std::to_string((int)db), 10); l.setPosition({x+2, yPos-12}); l.setFillColor(sf::Color::White); window.draw(l); }
    double startFreq = (double)cf - sr/2.0; int divs = (int)(w / 100); if (divs < 4) divs = 4;
    for (int i = 0; i <= divs; i++) { float xPos = x + (w/divs)*i; sf::RectangleShape line({1, h}); line.setPosition({xPos, y}); line.setFillColor(sf::Color(100, 100, 100, 150)); window.draw(line); double freqAtPoint = startFreq + (double)sr * ((double)i / divs); std::string freqStr = formatHz((long long)freqAtPoint); if(freqStr.size() > 4) freqStr = freqStr.substr(0, freqStr.size()-4); sf::Text l(font, freqStr, 10); sf::FloatRect b = l.getLocalBounds(); l.setPosition({xPos - b.size.x/2, y + h - 15}); l.setFillColor(sf::Color::White); window.draw(l); }
}
...
void dspWorker(std::atomic<bool>& running, SharedData& shared, AudioSink& audio) {
    ...
    while (running) {
        ...
        double targetFreqPct, bw; float vol, rfGainReq; bool muted; Mode mode; bool play, aprsActive; float minDb, maxDb; bool doRecord; RecMode rMode; std::string rPath; float sqThr; bool stereo; double seekReq = -1.0;
...
int main() {
    ...
    // --- HARDWARE CONTROLS DECLARATION (Keep shared_ptrs alive) ---
    auto modRadio = sidebar.addModule("Radio Control");
    
    // Gain & AGC (Always available for HW)
    ...
    // Filters & Bias-T
    // Split Bias-T out so it can be added to both RTL and SDRPlay if needed
    ...
    // --- FUNCTION TO DYNAMICALLY REBUILD UI ---
    auto updateRadioControls = [&](int sourceIdx) {
        // Clear existing widgets in the module
        modRadio->widgets.clear();
        
        // 0=File, 1=RTL, 2=SDRPlay
        if (sourceIdx != 0) {
            // Common Hardware Controls
            modRadio->addWidget(slGain);
            modRadio->addWidget(chkAgc);
        }

        if (sourceIdx == 1) { // RTL-SDR
            modRadio->addWidget(lblDirectSamp);
            modRadio->addWidget(ddDirectSamp);
            modRadio->addWidget(chkBiasT);
        } 
        else if (sourceIdx == 2) { // SDRPlay
            modRadio->addWidget(lblAntenna);
            modRadio->addWidget(ddAntenna);
            modRadio->addWidget(rowFilters); // FM/MW Notches
            modRadio->addWidget(chkBiasT);
        }

        // Force Sidebar to recalculate positions and scrollbar
    ...
    changeSource = [&](int sourceIdx, int rateIdx, std::string pathOverride) {
        ...
        if (sourceIdx == 0) { newSource = std::make_shared<FileSource>(); std::string path = pathOverride.empty() ? "None" : pathOverride; success = newSource->open(path); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.currentFilename = path; } ddRate->setOptions({path.substr(path.find_last_of("/\\") + 1)}); }
    ...
    auto setMode = [&](Mode m, SdrButton* me) { 
        ...
        if (m == Mode::WFM) { slBW->setLimits(50000, 250000); slBW->setValueSilent(180000); sharedData.bandwidth = 180000; slBW->setText("Bandwidth: 180 kHz"); } 
    ...
    auto btnSetFolder = std::make_shared<SdrButton>(110, 25, "Output Folder...", font); btnSetFolder->onClick = [&](){ std::string p = selectFolderDialog(); if(!p.empty()) { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.recPath = p; } }; /*modRec->addWidget(btnSetFolder);*/
   rowRecMode->add(btnSetFolder);
    ...
    auto updateLayout = [&](int w, int h) {
        layout.winW = (float)w; layout.winH = (float)h; layout.sidebarX = layout.winW - SIDEBAR_W; layout.specW = layout.sidebarX; layout.specH = 250.0f; layout.waterfallH = layout.winH - TOP_BAR_H - layout.specH; if (layout.waterfallH < 100) layout.waterfallH = 100;
        topBar.setSize({layout.winW, (float)TOP_BAR_H}); freqVFO.setPosition(20, 4); btnTuningMode.setPosition(370, 10); btnPlay.setPosition(320, 10); volSlider.setPosition(layout.winW - 170, 10); btnMute.setPosition(layout.winW - 220, 10);
        ...
        wSpr.setPosition({0, TOP_BAR_H + layout.specH}); float scaleX = layout.specW / (float)INTERNAL_WATERFALL_WIDTH; wSpr.setScale({scaleX, 1.0f}); wSpr.setTextureRect(sf::IntRect({0, 0}, {INTERNAL_WATERFALL_WIDTH, (int)layout.waterfallH})); 
    };
    updateLayout(window.getSize().x, window.getSize().y);
    sf::Clock debouncer; sf::Clock interactionCooldown; bool isDraggingScale = false, isSpectrumDragging = false; float lastDragX = 0.0f; float aprsLogScrollOffset = 0.0f;
    auto applySpectrumTuning = [&](float mouseX) { bool isHw = false; double hwSampleRate = 2e6; { std::lock_guard<std::mutex> l(sourceMtx); if (currentSource) { isHw = currentSource->isHardware(); hwSampleRate = currentSource->getSampleRate(); } } double clickPct = mouseX / layout.specW; double offsetHz = (clickPct - 0.5) * hwSampleRate; long long clickedFreq = currentCenterFreq + (long long)offsetHz; long long step = STEP_VALUES[ddSnap->selectedIndex]; if (step > 0) { clickedFreq = (long long)std::round((double)clickedFreq / step) * step; double newOffset = (double)(clickedFreq - currentCenterFreq); clickPct = 0.5 + (newOffset / hwSampleRate); } clickPct = std::clamp(clickPct, 0.0, 1.0); if (stickyCenterMode && isHw) { pendingCenterFreq = clickedFreq; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; } freqVFO.setFrequency(clickedFreq); } else { { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = clickPct; } freqVFO.setFrequency(clickedFreq); } };
    ...
    while (window.isOpen()) {
        ...
        bool aprsOn = false; { std::lock_guard<std::mutex> l(sharedData.mtx); aprsOn = sharedData.aprsEnabled; }
        ...
        while (const std::optional<sf::Event> ev = window.pollEvent()) {
            ...
            if (const auto* mb = ev->getIf<sf::Event::MouseButtonPressed>()) { if (mb->button == sf::Mouse::Button::Left) { sf::Vector2f m = window.mapPixelToCoords(sf::Mouse::getPosition(window)); bool overTimeline = (timeSlider.isDragging || (m.y > layout.winH - 40)); bool inFreqScaleZone = (m.y >= TOP_BAR_H + layout.specH - 10 && m.y <= TOP_BAR_H + layout.specH + 20 && m.x < layout.specW); if (inFreqScaleZone && !overTimeline && !overAprs) { isDraggingScale = true; lastDragX = m.x; } else if (!overTimeline && !overAprs && m.x < layout.specW && m.y > TOP_BAR_H && m.y < (TOP_BAR_H + layout.specH + layout.waterfallH)) { isSpectrumDragging = true; applySpectrumTuning(m.x); } } } else if (const auto* mr = ev->getIf<sf::Event::MouseButtonReleased>()) { if (mr->button == sf::Mouse::Button::Left) { isSpectrumDragging = false; isDraggingScale = false; } } else if (const auto* mm = ev->getIf<sf::Event::MouseMoved>()) { sf::Vector2f m = window.mapPixelToCoords(mm->position); if (isSpectrumDragging) applySpectrumTuning(m.x); if (isDraggingScale) { float dx = lastDragX - m.x; lastDragX = m.x; double hzPerPx = hwSampleRate / layout.specW; long long shift = (long long)(dx * hzPerPx); long long nextCenter = currentCenterFreq + shift; if (nextCenter < 0) nextCenter = 0; currentCenterFreq = nextCenter; { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.centerFreq = nextCenter; } double currentPct = 0.0; { std::lock_guard<std::mutex> l(sharedData.mtx); currentPct = sharedData.tunedFreqPercent; } double offset = (currentPct - 0.5) * hwSampleRate; long long vfoFreq = nextCenter + (long long)offset; freqVFO.setFrequency(vfoFreq); if (debouncer.getElapsedTime().asMilliseconds() > TUNING_LATENCY_MS) { { std::lock_guard<std::mutex> l(sourceMtx); if (currentSource) currentSource->setCenterFrequency(nextCenter); } debouncer.restart(); } } 
        ...
        if (showHand && cursorHand) window.setMouseCursor(*cursorHand); else if(cursorArrow) window.setMouseCursor(*cursorArrow);

        std::vector<double> spectrum; std::vector<uint8_t> row; bool newRow = false; double tunePct = 0.5; Mode mode = Mode::NFM; { std::lock_guard<std::mutex> lock(sharedData.mtx); spectrum = sharedData.fftSpectrum; if (sharedData.newWaterfallData) { row = sharedData.waterfallRow; sharedData.newWaterfallData = false; newRow = true; } tunePct = sharedData.tunedFreqPercent; mode = sharedData.mode; }
        ...
        drawGrid(window, font, 0, TOP_BAR_H, layout.specW, layout.specH, cf, hwSampleRate, slMinDb->currentVal, slMaxDb->currentVal);
       // --- 1. FFT (Moving Average) ---
        ...
        // --- DRAW GRADIENT ---
        sf::VertexArray fillArea(sf::PrimitiveType::TriangleStrip, spectrum.size() * 2);
        sf::VertexArray outline(sf::PrimitiveType::LineStrip, spectrum.size());

        for (size_t i = 0; i < spectrum.size(); i++) {
            float x = (float)i / spectrum.size() * layout.specW;
            ...
            sf::Color topColor(0, 180, 255, 140); 
            sf::Color bottomColor(0, 0, 0, 0);
            fillArea[2 * i].position = {x, topY};
            fillArea[2 * i].color = topColor;
            fillArea[2 * i + 1].position = {x, bottomY};
            fillArea[2 * i + 1].color = bottomColor;
            outline[i].position = {x, topY};
            outline[i].color = sf::Color(150, 230, 255, 255);
        }

        window.draw(fillArea);
        window.draw(outline);
        wSpr.setTextureRect(sf::IntRect({0, 0}, {INTERNAL_WATERFALL_WIDTH, (int)layout.waterfallH})); window.draw(wSpr);
        ...
        sf::RectangleShape tunerRect; float bwPixels = (slBW->currentVal / hwSampleRate) * layout.specW; if (bwPixels < 2.0f) bwPixels = 2.0f; float rectX = tunePct * layout.specW - bwPixels / 2.0f; if (mode == Mode::USB) rectX += bwPixels / 2.0f; if (mode == Mode::LSB) rectX -= bwPixels / 2.0f; tunerRect.setSize({bwPixels, (float)layout.specH}); tunerRect.setPosition({rectX, (float)TOP_BAR_H}); tunerRect.setFillColor(mode == Mode::OFF ? sf::Color(50, 50, 50, 40) : sf::Color(200, 200, 200, 50)); window.draw(tunerRect);
        sf::VertexArray centerLine(sf::PrimitiveType::Lines, 2); float centerX = tunePct * layout.specW; centerLine[0].position = {centerX, (float)TOP_BAR_H}; centerLine[0].color = sf::Color::Red; centerLine[1].position = {centerX, (float)layout.specH + TOP_BAR_H}; centerLine[1].color = sf::Color::Red; window.draw(centerLine);
