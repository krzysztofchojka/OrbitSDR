...
int main() {
    ...
    auto btnNFM = std::make_shared<SdrButton>(36, 25, "NFM", font); auto btnAM = std::make_shared<SdrButton>(36, 25, "AM", font); auto btnWFM = std::make_shared<SdrButton>(36, 25, "WFM", font); auto btnUSB = std::make_shared<SdrButton>(36, 25, "USB", font); auto btnLSB = std::make_shared<SdrButton>(36, 25, "LSB", font); auto btnRAW = std::make_shared<SdrButton>(42, 25, "RAW", font); auto btnOFF = std::make_shared<SdrButton>(36, 25, "OFF", font);
    ...
    auto ddSourceType = std::make_shared<Dropdown>(SIDEBAR_W - 40, 25.0f, font); ddSourceType->setOptions({"File Source", "RTL-SDR", "SDRPlay", "Sound Card (Line-In)"}); modSource->addWidget(std::make_shared<Label>("Source Type:", font)); modSource->addWidget(ddSourceType);
    ...
    int currentSourceType = 0; sf::Clock gainDebouncer; float pendingRfGain = -999.0f; 
    ...
    slGain->onChange = [&](float v) { 
        ...
        slGain->setText(ss.str()); pendingRfGain = v;
    }; 
    ...
    auto updateRadioControls = [&](int sourceIdx) {
        ...
        if (sourceIdx == 1) { modRadio->addWidget(lblDirectSamp); modRadio->addWidget(ddDirectSamp); modRadio->addWidget(chkBiasT); ddDirectSamp->setSelection(hwState.directSampling); chkBiasT->checked = hwState.biasT; } 
        else if (sourceIdx == 2) { modRadio->addWidget(lblAntenna); modRadio->addWidget(ddAntenna); modRadio->addWidget(rowFilters); modRadio->addWidget(chkBiasT); ddAntenna->setSelection(hwState.antennaIndex); chkFmNotch->checked = hwState.fmNotch; chkMwNotch->checked = hwState.mwNotch; chkBiasT->checked = hwState.biasT; }
        sidebar.recalculateLayout(); sidebar.updateStyle();
    };
    updateRadioControls(0);

    long long currentCenterFreq = savedFreq; long long pendingCenterFreq = 0;
    ...
    std::function<void(int, std::string, int, std::string)> doOpenSource = 
        ...
        if (sourceIdx == 0) { 
            ...
            std::string fileName = path.substr(path.find_last_of("/\\") + 1); ddRate->setOptions({fileName, "[ Change File... ]"}); ddRate->setSelection(0);
            long long f = 0; std::regex freqRegex(R"(_(\d+)Hz_)"); std::smatch match; if (std::regex_search(path, match, freqRegex)) { try { f = std::stoll(match[1]); } catch(...) {} }
            currentCenterFreq = f; freqVFO.setFrequency(f); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.centerFreq = f; } 
        }
        else if (sourceIdx == 1) { 
            ...
            freqVFO.setFrequency(currentCenterFreq); newSource->setCenterFrequency(currentCenterFreq); 
            ...
            slGain->setLimits(0.0f, 49.6f); slGain->setText("RF Gain: AGC"); newSource->setHardwareOption("direct_sampling", hwState.directSampling); newSource->setHardwareOption("bias_t", hwState.biasT ? 1 : 0);
        }
        else if (sourceIdx == 2) { 
            ...
            newSource->setHardwareOption("antenna", hwState.antennaIndex); newSource->setHardwareOption("fm_notch", hwState.fmNotch ? 1 : 0); newSource->setHardwareOption("mw_notch", hwState.mwNotch ? 1 : 0); newSource->setHardwareOption("bias_t", hwState.biasT ? 1 : 0);
        }
        else if (sourceIdx == 3) {
            targetRate = 48000; auto audioIn = std::make_shared<AudioCaptureSource>(); ddRate->setOptions(audioIn->getAvailableSampleRatesText()); ddRate->setSelection(0); success = audioIn->open(deviceID, targetRate); newSource = audioIn;
            currentCenterFreq = 0; freqVFO.setFrequency(currentCenterFreq);
            { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; sharedData.centerFreq = 0; }
            slGain->setEnabled(false); chkAgc->checked = false;
        }
    ...
    std::function<void()> refreshSourceUI = [&]() {
        ...
        if (ddSourceType->selectedIndex == 0) { 
            modSource->addWidget(std::make_shared<Label>("Loaded File:", font)); std::string fname; { std::lock_guard<std::mutex> l(sharedData.mtx); fname = sharedData.currentFilename; }
            if (fname == "None" || fname.empty()) { ddRate->setOptions({"[ Select File... ]"}); } else { std::string shortName = fname.substr(fname.find_last_of("/\\") + 1); ddRate->setOptions({shortName, "[ Change File... ]"}); }
            modSource->addWidget(ddRate); 
        } 
        else { modSource->addWidget(lblDevice); modSource->addWidget(ddDevice); modSource->addWidget(btnRefresh); modSource->addWidget(std::make_shared<Label>("Sample Rate:", font)); modSource->addWidget(ddRate); }
        sidebar.recalculateLayout(); sidebar.updateStyle();
    };

    ddSourceType->onChange = [&](int idx) {
        if (idx == 0) { std::string path = openFileDialog(); if (!path.empty()) { doOpenSource(0, "", 0, path); refreshSourceUI(); } else refreshSourceUI(); } 
        else { cachedDevices.clear(); std::vector<std::string> names; if (idx == 1) cachedDevices = RtlSdrSource::getDeviceList(); else if (idx == 2) cachedDevices = SdrPlaySource::getDeviceList(); else if (idx == 3) cachedDevices = AudioCaptureSource::getDeviceList(); for(auto& d : cachedDevices) names.push_back(d.name); if (names.empty()) names.push_back("No Devices Found"); ddDevice->setOptions(names); refreshSourceUI(); if (!cachedDevices.empty()) { doOpenSource(idx, cachedDevices[0].id, 0, ""); } }
    };
    ...
    btnRefresh->onClick = [&]() { 
        ...
        if (idx == 1) cachedDevices = RtlSdrSource::getDeviceList(); else if (idx == 2) cachedDevices = SdrPlaySource::getDeviceList(); else if (idx == 3) cachedDevices = AudioCaptureSource::getDeviceList();
    ...
    ddRate->onChange = [&](int idx) { 
        if (ddSourceType->selectedIndex == 0) {
            std::string currentOpt = ""; if(idx >= 0 && idx < ddRate->options.size()) currentOpt = ddRate->options[idx];
            if (currentOpt == "[ Select File... ]" || currentOpt == "[ Change File... ]") { std::string path = openFileDialog(); if (!path.empty()) doOpenSource(0, "", 0, path); else refreshSourceUI(); }
        }
        else if (ddSourceType->selectedIndex != 0 && !cachedDevices.empty()) { doOpenSource(ddSourceType->selectedIndex, cachedDevices[ddDevice->selectedIndex].id, idx, ""); } 
    };
    ...
    auto setMode = [&](Mode m, SdrButton* me) { 
        if (chkAprs->checked) { chkAprs->checked = false; { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.aprsEnabled = false; } } 
        if (chkAIS->checked) { chkAIS->checked = false; { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.activeDecoder = nullptr; ptrAIS->enabled = false; } }
        rowModes->setEnabled(true); btnNFM->setActive(false); btnAM->setActive(false); btnWFM->setActive(false); btnUSB->setActive(false); btnLSB->setActive(false); btnRAW->setActive(false); btnOFF->setActive(false); me->setActive(true); 
        ...
        else if (m == Mode::RAW) { slBW->setLimits(1000, 200000); slBW->setValueSilent(48000); sharedData.bandwidth = 48000; slBW->setText("Bandwidth: 48.0 kHz"); }
    ...
    btnNFM->onClick = [&](){ setMode(Mode::NFM, btnNFM.get()); }; btnAM->onClick = [&](){ setMode(Mode::AM, btnAM.get()); }; btnWFM->onClick = [&](){ setMode(Mode::WFM, btnWFM.get()); }; btnUSB->onClick = [&](){ setMode(Mode::USB, btnUSB.get()); }; btnLSB->onClick = [&](){ setMode(Mode::LSB, btnLSB.get()); }; btnRAW->onClick = [&](){ setMode(Mode::RAW, btnRAW.get()); }; btnOFF->onClick = [&](){ setMode(Mode::OFF, btnOFF.get()); };
    
    chkAprs->onToggle = [&](bool b) { 
        { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.aprsEnabled = b; } rowModes->setEnabled(!b); 
        if(b && chkAIS->checked) { chkAIS->checked = false; { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.activeDecoder = nullptr; ptrAIS->enabled = false; } }
        if (b) {
            { std::lock_guard<std::mutex> l(sharedData.mtx); previousMode = sharedData.mode; }
            bool isAudioIn = (currentSourceType == 3);
            btnNFM->setActive(!isAudioIn); btnAM->setActive(false); btnWFM->setActive(false); btnUSB->setActive(false); btnLSB->setActive(false); btnRAW->setActive(isAudioIn); btnOFF->setActive(false);
            { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.mode = isAudioIn ? Mode::RAW : Mode::NFM; sharedData.bandwidth = isAudioIn ? 48000.0f : 12500.0f; }
            if (isAudioIn) { slBW->setLimits(1000, 200000); slBW->setValueSilent(48000); slBW->setText("Bandwidth: 48.0 kHz"); } else { slBW->setLimits(4000, 40000); slBW->setValueSilent(12500); slBW->setText("Bandwidth: 12.5 kHz"); }
        } else { 
    ...
    modDecoders->addWidget(chkAprs); btnCopyAprs->onClick = [&](){ std::lock_guard<std::mutex> l(sharedData.mtx); sf::Clipboard::setString(sharedData.lastAprs.raw); }; modDecoders->addWidget(btnCopyAprs);

    chkAIS->onToggle = [&](bool b) {
        std::lock_guard<std::mutex> l(sharedData.mtx);
        if(b && chkAprs->checked) { chkAprs->checked = false; sharedData.aprsEnabled = false; }
        rowModes->setEnabled(!b);
        if (b) {
            sharedData.activeDecoder = ptrAIS; ptrAIS->enabled = true; previousMode = sharedData.mode;
            bool isAudioIn = (currentSourceType == 3);
            sharedData.mode = isAudioIn ? Mode::RAW : Mode::NFM; sharedData.bandwidth = isAudioIn ? 48000.0f : 25000.0f;
            btnNFM->setActive(!isAudioIn); btnAM->setActive(false); btnWFM->setActive(false); btnUSB->setActive(false); btnLSB->setActive(false); btnRAW->setActive(isAudioIn); btnOFF->setActive(false);
            if (isAudioIn) { slBW->setLimits(1000, 200000); slBW->setValueSilent(48000); slBW->setText("Bandwidth: 48.0 kHz"); } else { slBW->setLimits(4000, 40000); slBW->setValueSilent(25000); slBW->setText("Bandwidth: 25.0 kHz"); }
        } else { sharedData.activeDecoder = nullptr; ptrAIS->enabled = false; }
    };
    auto aisUiRestore = [&](bool b) { if(!b) { if(previousMode == Mode::NFM) setMode(Mode::NFM, btnNFM.get()); else if(previousMode == Mode::AM) setMode(Mode::AM, btnAM.get()); else if(previousMode == Mode::WFM) setMode(Mode::WFM, btnWFM.get()); else if (previousMode == Mode::RAW) setMode(Mode::RAW, btnRAW.get()); else if(previousMode == Mode::USB) setMode(Mode::USB, btnUSB.get()); else if(previousMode == Mode::LSB) setMode(Mode::LSB, btnLSB.get()); else setMode(Mode::OFF, btnOFF.get()); } };
    auto originalAisToggle = chkAIS->onToggle; chkAIS->onToggle = [=](bool b) { originalAisToggle(b); aisUiRestore(b); };
    ...
    ddTheme->onChange = [&](int idx) { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.waterfallTheme = idx; Theme::setTheme(idx); freqVFO.updateStyle(); btnTuningMode.updateStyle(); btnPlay.updateStyle(); btnMute.updateStyle(); volSlider.updateStyle(); timeSlider.updateStyle(); sidebar.updateStyle(); }; 
    ...
    btnRecAudio->onClick = [&](){ btnRecAudio->setActive(true); btnRecIQ->setActive(false); std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.recMode = RecMode::AUDIO; }; 
    btnRecIQ->onClick = [&](){ btnRecAudio->setActive(false); btnRecIQ->setActive(true); std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.recMode = RecMode::BASEBAND; }; 
    rowRecMode->add(btnRecAudio); rowRecMode->add(btnRecIQ); modRec->addWidget(rowRecMode); 
    auto lblRecPath = std::make_shared<Label>("Path: ...", font, 11, sf::Color(150,150,150)); modRec->addWidget(lblRecPath);
    auto btnSetFolder = std::make_shared<SdrButton>(SIDEBAR_W - 40, 25, "Change Folder...", font); 
    btnSetFolder->onClick = [&](){ std::string p = selectFolderDialog(); if(!p.empty()) { { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.recPath = p; } lblRecPath->setText("Path: " + truncatePath(p, 25)); } }; 
    ...
    btnRecToggle->onClick = [&](){ std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.isRecording = !sharedData.isRecording; if(sharedData.isRecording) { btnRecToggle->setText("STOP RECORDING"); btnRecToggle->setColor(sf::Color(180, 50, 50)); } else { btnRecToggle->setText("START RECORDING"); btnRecToggle->setColor(sf::Color(40, 40, 45)); } }; 
    modRec->addWidget(btnRecToggle);

    const int MINI_W = 280; const int MINI_H = 120; std::vector<uint8_t> miniWaterfall(MINI_W * MINI_H * 4, 0); sf::Texture miniTex; (void)miniTex.resize({(unsigned int)MINI_W, (unsigned int)MINI_H}); sf::Sprite miniSpr(miniTex);
    auto modInspector = sidebar.addModule("Signal Inspector"); auto spacer = std::make_shared<Spacer>(130.0f); modInspector->addWidget(spacer);
    std::vector<std::uint8_t> waterfall(INTERNAL_WATERFALL_WIDTH * WATERFALL_HEIGHT_PX * 4, 0); sf::Texture wTex; if(!wTex.resize({INTERNAL_WATERFALL_WIDTH, WATERFALL_HEIGHT_PX})) return 1; wTex.setSmooth(true); sf::Sprite wSpr(wTex); 
    ...
    for (auto& mod : sidebar.modules) { if (mod->title == "Source / Input") mod->isOpen = true; else mod->isOpen = settingsMgr.getBool("sidebar_" + mod->title, mod->isOpen); }
    int savedTheme = settingsMgr.getInt("theme", 0); ddTheme->setSelection(savedTheme); ddTheme->onChange(savedTheme); 
    slMinDb->setValueSilent(settingsMgr.getFloat("min_db", -90.0f)); slMinDb->onChange(slMinDb->currentVal);
    slMaxDb->setValueSilent(settingsMgr.getFloat("max_db", 0.0f)); slMaxDb->onChange(slMaxDb->currentVal);
    slZoom->setValueSilent(settingsMgr.getFloat("zoom", 1.0f)); slZoom->onChange(slZoom->currentVal);
    ddSnap->setSelection(settingsMgr.getInt("step_index", 5));
    int savedMode = settingsMgr.getInt("mode_index", 1); Mode mEnum = (Mode)savedMode; if (mEnum == Mode::NFM) setMode(Mode::NFM, btnNFM.get()); else if (mEnum == Mode::AM) setMode(Mode::AM, btnAM.get()); else if (mEnum == Mode::WFM) setMode(Mode::WFM, btnWFM.get()); else if (mEnum == Mode::USB) setMode(Mode::USB, btnUSB.get()); else if (mEnum == Mode::LSB) setMode(Mode::LSB, btnLSB.get()); else if (mEnum == Mode::RAW) setMode(Mode::RAW, btnRAW.get()); else setMode(Mode::OFF, btnOFF.get());
    float savedBw = settingsMgr.getFloat("bandwidth", 12500.0f); slBW->setValueSilent(savedBw); slBW->onChange(savedBw);
    float savedSq = settingsMgr.getFloat("squelch", -100.0f); slSq->setValueSilent(savedSq); slSq->onChange(savedSq);
    bool savedStereo = settingsMgr.getBool("stereo", false); chkStereo->checked = savedStereo; chkStereo->onToggle(savedStereo);
    float savedGain = settingsMgr.getFloat("rf_gain", -1.0f); if (savedGain < 0) { chkAgc->checked = true; chkAgc->onToggle(true); } else { chkAgc->checked = false; chkAgc->onToggle(false); slGain->setValueSilent(savedGain); slGain->onChange(savedGain); }
    hwState.fmNotch = settingsMgr.getBool("hw_fm_notch", false); hwState.mwNotch = settingsMgr.getBool("hw_mw_notch", false); hwState.biasT = settingsMgr.getBool("hw_bias_t", false); hwState.directSampling = settingsMgr.getInt("hw_direct_samp", 0); hwState.antennaIndex = settingsMgr.getInt("hw_antenna", 0);
    int savedSourceIdx = settingsMgr.getInt("source_type", 0); if (savedSourceIdx != 0) { ddSourceType->setSelection(savedSourceIdx); refreshSourceUI(); }
    std::string savedRecPath = settingsMgr.getString("rec_path", ""); if (savedRecPath.empty()) { savedRecPath = getDefaultRecordingPath(); }
    { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.recPath = savedRecPath; } lblRecPath->setText("Path: " + truncatePath(savedRecPath, 25));

    sf::Clock debouncer; sf::Clock interactionCooldown; bool isDraggingScale = false, isSpectrumDragging = false; bool isDraggingBW = false; float dragStartBW = 0.0f; float dragStartMouseX = 0.0f; float lastDragX = 0.0f; float aprsLogScrollOffset = 0.0f; bool isPanningView = false;
    bool showAprsModal = false; AprsLastPacket selectedAprsPacket; SdrButton btnModalClose(80, 30, "Close", font); SdrButton btnModalCopy(80, 30, "Copy", font);
    btnModalClose.onClick = [&]() { showAprsModal = false; }; btnModalCopy.onClick = [&]() { sf::Clipboard::setString(selectedAprsPacket.raw); };
    ...
    while (window.isOpen()) {
        ...
        freqVFO.setEnabled(true); if (!ddSourceType->isOpen && !ddRate->isOpen && !ddDevice->isOpen) freqVFO.update(window); volSlider.update(window); if (std::abs(volSlider.currentVal - sharedData.volume) > 0.01f) { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.volume = volSlider.currentVal; }
        if (currentSourceType == 0) { timeSlider.update(window); if (timeSlider.isDragging) { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.pendingSeekRequest = timeSlider.currentVal; } else { timeSlider.currentVal = prog; timeSlider.updateHandlePos(); } }
        ...
        double visibleFraction = 1.0 / currentZoom; double viewStartPct = viewCenter - visibleFraction / 2.0; double viewEndPct = viewCenter + visibleFraction / 2.0;
        ...
        float tunerPosInView = (tunePct - viewStartPct) / viewWidthPct; float visualCenterX = tunerPosInView * layout.specW; float bwPct = (slBW->currentVal / hwSampleRate); float bwPixels = (bwPct / viewWidthPct) * layout.specW;

        auto applySpectrumTuning = [&](float mouseX) { 
            bool isHwLocal = false; double sr = 2e6; { std::lock_guard<std::mutex> l(sourceMtx); if (currentSource) { isHwLocal = currentSource->isHardware(); sr = currentSource->getSampleRate(); } } 
            double clickViewPct = mouseX / layout.specW; double clickedAbsolutePct = viewStartPct + clickViewPct * viewWidthPct; double offsetHz = (clickedAbsolutePct - 0.5) * sr;
            long long clickedFreq = currentCenterFreq + (long long)offsetHz; long long step = STEP_VALUES[ddSnap->selectedIndex]; 
            if (step > 0) { clickedFreq = (long long)std::round((double)clickedFreq / step) * step; double newOffset = (double)(clickedFreq - currentCenterFreq); clickedAbsolutePct = 0.5 + (newOffset / sr); } 
            if (stickyCenterMode && isHwLocal) { pendingCenterFreq = clickedFreq; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; } freqVFO.setFrequency(clickedFreq); } 
            else { { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = clickedAbsolutePct; } freqVFO.setFrequency(clickedFreq); } 
        };

        if (pendingRfGain > -900.0f && gainDebouncer.getElapsedTime().asMilliseconds() > GAIN_LATENCY_MS) { { std::lock_guard<std::mutex> l(sourceMtx); if (currentSource && currentSource->isHardware()) { currentSource->setGain((int)pendingRfGain); } } pendingRfGain = -999.0f; gainDebouncer.restart(); }

        while (const std::optional<sf::Event> ev = window.pollEvent()) {
            if (ev->is<sf::Event::Closed>()) window.close();
            if (showAprsModal) { btnModalClose.handleEvent(*ev, window); btnModalCopy.handleEvent(*ev, window); if (const auto* mb = ev->getIf<sf::Event::MouseButtonPressed>()) { sf::Vector2f m = window.mapPixelToCoords(sf::Mouse::getPosition(window)); float mx = (layout.winW - 600) / 2; float my = (layout.winH - 400) / 2; sf::FloatRect modalRect({mx, my}, {600.f, 400.f}); if (!modalRect.contains(m)) showAprsModal = false; } continue; }
            if (sidebar.handleEvent(*ev, window)) continue; 
            
            bool topBarHandled = false; if (volSlider.handleEvent(*ev, window)) topBarHandled = true; if (slZoom->handleEvent(*ev, window)) topBarHandled = true; if (currentSourceType == 0 && timeSlider.handleEvent(*ev, window)) topBarHandled = true;
            ...
            if (freqVFO.handleEvent(*ev)) { 
                topBarHandled = true; long long targetVFO = freqVFO.getFrequency(); double halfBW = hwSampleRate / 2.0; double minF = (double)currentCenterFreq - halfBW; double maxF = (double)currentCenterFreq + halfBW;
                if (stickyCenterMode && isHw) { pendingCenterFreq = targetVFO; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; } } 
                else { if (targetVFO > maxF || targetVFO < minF) { if (isHw) { pendingCenterFreq = targetVFO; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; } } else { if (targetVFO > maxF) targetVFO = (long long)maxF; if (targetVFO < minF) targetVFO = (long long)minF; double newOffset = (double)(targetVFO - currentCenterFreq); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5 + (newOffset / hwSampleRate); } } } else { double newOffset = (double)(targetVFO - currentCenterFreq); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5 + (newOffset / hwSampleRate); } } }
                if (targetVFO != freqVFO.getFrequency()) freqVFO.setFrequency(targetVFO);
                if (stickyCenterMode && isHw) { pendingCenterFreq = targetVFO; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; } } 
                else { double newOffset = (double)(targetVFO - currentCenterFreq); double pct = 0.5 + (newOffset / hwSampleRate); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = pct; } } 
            }
            ...
            if (const auto* scroll = ev->getIf<sf::Event::MouseWheelScrolled>()) { 
                if (overAprs) { aprsLogScrollOffset += scroll->delta * 20.0f; if (aprsLogScrollOffset > 0.0f) aprsLogScrollOffset = 0.0f; } 
                else { sf::Vector2f m = window.mapPixelToCoords(sf::Vector2i((int)scroll->position.x, (int)scroll->position.y)); if (m.y > TOP_BAR_H && m.x < layout.specW) { long long step = STEP_VALUES[ddSnap->selectedIndex]; if (step == 0) step = 100; long long current = freqVFO.getFrequency(); long long rawNext = current + (long long)(scroll->delta * step); if (step > 0) { long long remainder = rawNext % step; if (remainder > step / 2) rawNext += (step - remainder); else rawNext -= remainder; } if (rawNext < 0) rawNext = 0; freqVFO.setFrequency(rawNext); if (stickyCenterMode && isHw) { pendingCenterFreq = rawNext; debouncer.restart(); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = 0.5; sharedData.viewCenterPct = 0.5; } } else { double newOffset = (double)(rawNext - currentCenterFreq); double clickPct = 0.5 + (newOffset / hwSampleRate); clickPct = std::clamp(clickPct, 0.0, 1.0); { std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.tunedFreqPercent = clickPct; } } } } 
            }
            if (const auto* mb = ev->getIf<sf::Event::MouseButtonPressed>()) { 
                sf::Vector2f m = window.mapPixelToCoords(sf::Mouse::getPosition(window)); 
                if (mb->button == sf::Mouse::Button::Right) { bool inFreqScaleZone = (m.y > TOP_BAR_H && m.y < (TOP_BAR_H + layout.specH + layout.waterfallH) && m.x < layout.specW); if (inFreqScaleZone && !overAprs) { isPanningView = true; lastDragX = m.x; } }
                else if (mb->button == sf::Mouse::Button::Left) { 
                    if (overAprs) { float logX = layout.specW - 300; float overlayY = layout.winH - 200.0f; float listTopY = overlayY + 25.0f; if (m.x > logX) { float relY = m.y - listTopY - aprsLogScrollOffset; int index = (int)(relY / 16.0f); std::lock_guard<std::mutex> l(sharedData.mtx); if (index >= 0 && index < sharedData.aprsHistory.size()) { selectedAprsPacket = sharedData.aprsHistory[index]; showAprsModal = true; } } }
                    bool overTimeline = (currentSourceType == 0) && (timeSlider.isDragging || (m.y > layout.winH - 40)); bool inFreqScaleZone = (m.y >= TOP_BAR_H + layout.specH - 10 && m.y <= TOP_BAR_H + layout.specH + 20 && m.x < layout.specW); 
                    float leftEdge = visualCenterX - bwPixels / 2.0f; float rightEdge = visualCenterX + bwPixels / 2.0f; if (mode == Mode::USB) { leftEdge = visualCenterX; rightEdge = visualCenterX + bwPixels; } if (mode == Mode::LSB) { leftEdge = visualCenterX - bwPixels; rightEdge = visualCenterX; }
                    bool hitEdge = (std::abs(m.x - leftEdge) < 6 || std::abs(m.x - rightEdge) < 6) && (m.y > TOP_BAR_H && m.y < TOP_BAR_H + layout.specH); 
                    if (hitEdge && !overAprs) { isDraggingBW = true; dragStartBW = slBW->currentVal; dragStartMouseX = m.x; } else if (inFreqScaleZone && !overTimeline && !overAprs) { isDraggingScale = true; lastDragX = m.x; } else if (!overTimeline && !overAprs && m.x < layout.specW && m.y > TOP_BAR_H && m.y < (TOP_BAR_H + layout.specH + layout.waterfallH)) { isSpectrumDragging = true; applySpectrumTuning(m.x); } 
                } 
                ...
                if (isPanningView) {
                    float dx = lastDragX - m.x; lastDragX = m.x; double pctShift = (dx / layout.specW) * viewWidthPct;
                    std::lock_guard<std::mutex> l(sharedData.mtx); sharedData.viewCenterPct += pctShift;
                    ...
                    float hzPerPx = (hwSampleRate * viewWidthPct) / layout.specW; float dist = std::abs(m.x - visualCenterX); float newBW = dist * hzPerPx * 2.0f; if (mode == Mode::USB || mode == Mode::LSB) newBW = dist * hzPerPx;
                    newBW = std::clamp(newBW, slBW->minVal, slBW->maxVal); slBW->setValueSilent(newBW); slBW->onChange(newBW); 
        ...
        std::vector<uint8_t> newRow; bool hasData = false; { std::lock_guard<std::mutex> l(sharedData.mtx); if (sharedData.newInspectorData) { newRow = sharedData.inspectorWaterfallRow; sharedData.newInspectorData = false; hasData = true; } }
        if (hasData) { if (newRow.size() == MINI_W * 4) { if (MINI_H > 1) { std::memmove(miniWaterfall.data() + MINI_W * 4, miniWaterfall.data(), (MINI_H - 1) * MINI_W * 4); } std::memcpy(miniWaterfall.data(), newRow.data(), MINI_W * 4); miniTex.update(miniWaterfall.data()); } else { std::cerr << "[GPU Error] Inspector size mismatch!\n"; } }

        std::vector<double> spectrum; std::vector<uint8_t> row; bool newRow2 = false; { std::lock_guard<std::mutex> lock(sharedData.mtx); spectrum = sharedData.fftSpectrum; if (sharedData.newWaterfallData) { row = sharedData.waterfallRow; sharedData.newWaterfallData = false; newRow2 = true; } }
        ...
        sf::Color coreColor = Theme::Accent; sf::Color glowColor = Theme::Glow; sf::Color bottomColor = Theme::AccentDim; bottomColor.a = 20;
        ...
        wSpr.setTextureRect(sf::IntRect({0, 0}, {INTERNAL_WATERFALL_WIDTH, (int)layout.waterfallH})); float scaleX = layout.specW / (float)INTERNAL_WATERFALL_WIDTH; wSpr.setScale({scaleX, 1.0f}); window.draw(wSpr);
        ...
        if (aprsOn) { 
            float aprsH = 200.0f; float overlayY = layout.winH - aprsH; sf::RectangleShape bg({layout.specW, aprsH}); bg.setPosition({0, overlayY}); bg.setFillColor(sf::Color(20, 20, 25, 240)); bg.setOutlineColor(sf::Color::White); bg.setOutlineThickness(1); window.draw(bg); 
            AprsLastPacket pkt; std::deque<AprsLastPacket> logs; { std::lock_guard<std::mutex> l(sharedData.mtx); pkt = sharedData.lastAprs; logs = sharedData.aprsHistory; } 
            float compassX = 60.0f; float compassY = overlayY + 70.0f; float compassR = 40.0f; sf::CircleShape compass(compassR); compass.setOrigin({compassR, compassR}); compass.setPosition({compassX, compassY}); compass.setFillColor(sf::Color::Transparent); compass.setOutlineColor(sf::Color(100,100,100)); compass.setOutlineThickness(2); window.draw(compass);
            sf::Text dirT(font, "N", 24); dirT.setScale({0.5f, 0.5f}); dirT.setFillColor(sf::Color::Yellow); dirT.setPosition({compassX - 4, compassY - compassR - 15}); window.draw(dirT); sf::RectangleShape northTick({2, 6}); northTick.setFillColor(sf::Color::Yellow); northTick.setPosition({compassX - 1, compassY - compassR}); window.draw(northTick);
            ...
            for(const auto& logPkt : logs) { float yPos = listTopY + aprsLogScrollOffset + (i*16.0f); if (yPos > listTopY - 10 && yPos < layout.winH - 5) { sf::FloatRect rowRect({logX, yPos}, {logW, 16.0f}); bool isHover = rowRect.contains(mousePos) && !showAprsModal; if (isHover) { sf::RectangleShape hl(sf::Vector2f(logW - 2, 16.0f)); hl.setPosition({logX + 1, yPos}); hl.setFillColor(sf::Color(50, 50, 60)); window.draw(hl); } std::string lShort = logPkt.timestamp + " " + logPkt.src + ">" + logPkt.dest; if(lShort.length()>35) lShort=lShort.substr(0,32)+"..."; sf::Text l(font, lShort, 24); l.setScale({0.5f, 0.5f}); l.setPosition({logX + 10, yPos}); if (isHover) l.setFillColor(sf::Color::White); else l.setFillColor(sf::Color(200,200,200)); window.draw(l); } i++; }
        }

        SDRModule* decoderUI = nullptr; { std::lock_guard<std::mutex> l(sharedData.mtx); decoderUI = sharedData.activeDecoder; }
        if (decoderUI && decoderUI->enabled) { float tableH = 250.0f; float tableY = layout.winH - tableH; decoderUI->draw(window, 0, tableY, layout.specW, tableH); }
        if (currentSourceType == 0) timeSlider.draw(window); sidebar.draw(window); window.draw(topBar); freqVFO.draw(window); btnPlay.draw(window); if (isHw) btnTuningMode.draw(window); btnMute.draw(window); volSlider.draw(window);
        
        if (showAprsModal) {
            float mw = 600; float mh = 400; float mx = (layout.winW - mw) / 2; float my = (layout.winH - mh) / 2;
            sf::RectangleShape modalBg({mw, mh}); modalBg.setPosition({mx, my}); modalBg.setFillColor(sf::Color(30, 30, 35)); modalBg.setOutlineThickness(2); modalBg.setOutlineColor(Theme::Accent); window.draw(modalBg);
            sf::Text title(font, "Packet Details", 36); title.setScale({0.5f, 0.5f}); title.setPosition({mx + 20, my + 15}); title.setFillColor(Theme::Accent); window.draw(title);
            float cx = mx + 20; float cy = my + 60;
            auto drawField = [&](std::string label, std::string val, sf::Color c = sf::Color::White) { sf::Text l(font, label, 24); l.setScale({0.5f, 0.5f}); l.setPosition({cx, cy}); l.setFillColor(sf::Color(150,150,150)); window.draw(l); sf::Text v(font, val.empty() ? "N/A" : val, 24); v.setScale({0.5f, 0.5f}); v.setPosition({cx + 120, cy}); v.setFillColor(c); window.draw(v); cy += 30; };
            drawField("Time:", selectedAprsPacket.timestamp); drawField("Source:", selectedAprsPacket.src, sf::Color::Green); drawField("Dest:", selectedAprsPacket.dest); drawField("Coords:", selectedAprsPacket.coords, sf::Color::Cyan); std::string crs = (selectedAprsPacket.course >= 0) ? std::to_string((int)selectedAprsPacket.course) + " deg" : ""; drawField("Course:", crs);
            sf::Text rawL(font, "Raw Packet:", 24); rawL.setScale({0.5f, 0.5f}); rawL.setPosition({cx, cy}); rawL.setFillColor(sf::Color(150,150,150)); window.draw(rawL); cy += 20;
            std::string wrappedRaw = wrapText(selectedAprsPacket.raw, font, 24, mw - 40); sf::Text rawV(font, wrappedRaw, 24); rawV.setScale({0.5f, 0.5f}); rawV.setPosition({cx, cy}); rawV.setFillColor(sf::Color::Yellow); window.draw(rawV);
            btnModalClose.setPosition(mx + mw - 100, my + mh - 50); btnModalClose.draw(window); btnModalCopy.setPosition(mx + 20, my + mh - 50); btnModalCopy.draw(window);
        }

        if (modInspector->isOpen) {
            float mx = modInspector->headerBg.getPosition().x + 10; float my = modInspector->headerBg.getPosition().y + 30; 
            miniSpr.setPosition({(float)mx, (float)my});
            sf::RectangleShape border({(float)MINI_W + 2, (float)MINI_H + 2}); border.setPosition({(float)mx - 1, (float)my - 1}); border.setFillColor(sf::Color::Transparent); border.setOutlineColor(sf::Color(100, 100, 100)); border.setOutlineThickness(1); window.draw(border); window.draw(miniSpr);
            sf::RectangleShape centerLine({2.0f, (float)MINI_H}); centerLine.setPosition({mx + MINI_W / 2.0f - 1.0f, my}); centerLine.setFillColor(sf::Color(255, 0, 0, 150)); window.draw(centerLine);
            sf::Text infoTxt(font, "Bandwidth Scope (" + std::to_string((int)slBW->currentVal) + " Hz)", 14); infoTxt.setPosition({(float)mx, (float)my - 16}); infoTxt.setScale({0.5f, 0.5f}); infoTxt.setFillColor(sf::Color::Yellow);
        }
        window.display();
    }
    ...
    saveMgr.set("theme", ddTheme->selectedIndex); saveMgr.set("zoom", slZoom->currentVal); saveMgr.set("min_db", slMinDb->currentVal); saveMgr.set("max_db", slMaxDb->currentVal); saveMgr.set("step_index", ddSnap->selectedIndex); saveMgr.set("mode_index", (int)sharedData.mode); saveMgr.set("bandwidth", slBW->currentVal); saveMgr.set("squelch", slSq->currentVal); saveMgr.set("stereo", chkStereo->checked); saveMgr.set("rf_gain", sharedData.rfGain); saveMgr.set("source_type", ddSourceType->selectedIndex); saveMgr.set("hw_fm_notch", hwState.fmNotch); saveMgr.set("hw_mw_notch", hwState.mwNotch); saveMgr.set("hw_bias_t", hwState.biasT); saveMgr.set("hw_direct_samp", hwState.directSampling); saveMgr.set("hw_antenna", hwState.antennaIndex);
    { std::lock_guard<std::mutex> l(sharedData.mtx); saveMgr.set("rec_path", sharedData.recPath); } saveMgr.set("frequency", (double)currentCenterFreq); saveMgr.save(settingsPath);
