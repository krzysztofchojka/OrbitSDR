...
inline void dspWorker(std::atomic<bool>& running, SharedData& shared, AudioSink& audio) {
    ...
    aprsDecoder.onMessage = [&](std::string msg) {
        ...
        AprsLastPacket pkt; parseAprsData(fullLog, pkt);
    ...
    double lastSampleRate = 0; std::vector<Complex> iqBuffer;
    ...
    int fftBufferIdx = 0; float smoothedRssi = -100.0f;
    WavWriter recorder; sf::Clock waterfallTimer;
    ...
    while (running) {
        ...
        if (src->didLoop()) {
            demod.clear(); 
            SDRModule* activeMod = nullptr; { std::lock_guard<std::mutex> l(shared.mtx); activeMod = shared.activeDecoder; }
        ...
        if (readCount > 0) {
            ...
            // Czerwona linia ZAWSZE oznacza nośną (0 Hz) w DSP. Usuwamy sztuczne przesunięcia!
            ...
            if (play) {
                ...
                if (sqThr > -99.0f) { if (smoothedRssi < sqThr) std::fill(audioData.begin(), audioData.end(), 0.0f); }

                float finalVol = muted ? 0.0f : vol;
                for (auto& s : audioData) { if (std::isnan(s) || std::isinf(s)) s = 0.0f; else s = std::clamp(s * finalVol, -1.0f, 1.0f); }
            ...
            if (waterfallTimer.getElapsedTime().asMilliseconds() > 33) {
                ...
                if (viewStartPct < 0.0) { viewStartPct = 0.0; viewEndPct = visibleFraction; }
                if (viewEndPct > 1.0) { viewEndPct = 1.0; viewStartPct = 1.0 - visibleFraction; }
