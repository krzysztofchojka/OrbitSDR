...
public:
    ...
    void layout(float x, float y) {
        headerBg.setPosition({x + 5, y});
        headerText.setPosition({x + 25, y + 6});
        arrowText.setPosition({x + 10, y + 5});
        arrowText.setString(isOpen ? "v" : ">");

        if (isOpen) {
            float cy = y + 35.0f;
            for (auto& w : widgets) {
                w->setPosition(x + 15, cy);
                cy += w->getHeight() + 5.0f;
            }
        }
    }
    ...
    void draw(sf::RenderWindow& win) {
        ...
        win.draw(headerText);
        win.draw(arrowText);
        if (isOpen) for (auto& w : widgets) w->draw(win);
    }

    void drawOverlay(sf::RenderWindow& win) {
        if (isOpen) for (auto& w : widgets) w->drawOverlay(win);
    }
    ...
    bool isMouseOver(const sf::RenderWindow& win) const {
        ...
        if (isOpen) {
            for (const auto& w : widgets) if(w->isMouseOver(win)) return true;
        }
        return false;
    }
    ...
    bool handleEvent(const sf::Event& ev, const sf::RenderWindow& win) {
        if (const auto* mb = ev.getIf<sf::Event::MouseButtonPressed>()) {
            if (mb->button == sf::Mouse::Button::Left) {
                sf::Vector2f m = win.mapPixelToCoords(sf::Mouse::getPosition(win));
                if (headerBg.getGlobalBounds().contains(m)) {
                    isOpen = !isOpen;
                    return true;
                }
            }
        ...
        if (isOpen) {
            for (auto it = widgets.rbegin(); it != widgets.rend(); ++it) {
                if ((*it)->handleEvent(ev, win)) return true;
            }
        }
        return false;
    }
    ...
    void update(const sf::RenderWindow& win) {
        if (isOpen) for (auto& w : widgets) w->update(win);
    }
};
...
public:
    ...
    const sf::Font& font;
    sf::RectangleShape sbTrack, sbThumb;
    ...
    std::shared_ptr<Module> addModule(std::string title) {
        auto m = std::make_shared<Module>(title, width - 20.0f, font);
        modules.push_back(m);
        return m;
    }

    void setGeometry(float _x, float _y, float _h) {
        x = _x;
        y = _y;
        height = _h;
        sbTrack.setPosition({x + width - 12, y});
        sbTrack.setSize({12, height});
        recalculateLayout();
    }
    ...
    bool handleEvent(const sf::Event& ev, const sf::RenderWindow& win) {
        ...
        if (const auto* sc = ev.getIf<sf::Event::MouseWheelScrolled>()) {
            ...
            if (m.x > x && m.x < x + width) {
                scrollOffset -= sc->delta * 30.0f;
                if (scrollOffset < 0) scrollOffset = 0;
                float maxS = std::max(0.0f, totalContentHeight - height);
        ...
        sf::Vector2f m = win.mapPixelToCoords(sf::Mouse::getPosition(win));
        bool isMouseInside = (m.x > x && m.x < x + width && m.y > y && m.y < y + height);
        bool isRelease = ev.is<sf::Event::MouseButtonReleased>();

        // ZAWSZE propaguj zdarzenie puszczenia myszy, żeby widgety mogły się odblokować
        if (isMouseInside || isRelease) {
            if (isMouseInside && !isRelease) captured = true;
            for (auto& mod : modules) {
    ...
    void update(const sf::RenderWindow& win) {
        for(auto& m : modules) m->update(win);
    }
    ...
    void draw(sf::RenderWindow& win) {
        ...
        for (auto& m : modules) {
            float mt = m->headerBg.getPosition().y;
            float mb = mt + m->getTotalHeight();
            if (mb > y && mt < y + height) m->draw(win);
        }
        ...
        for (auto& m : modules) {
            float mt = m->headerBg.getPosition().y;
            float mb = mt + m->getTotalHeight();
            if (mb > y && mt < y + height) m->drawOverlay(win);
        }
    }
};