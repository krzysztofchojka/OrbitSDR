...
public:
    ...
    Module(std::string t, float w, const sf::Font& font) : title(t), width(w), headerText(font, t, 11), arrowText(font, "v", 12) {
        headerBg.setSize({width, 25.0f}); headerBg.setFillColor(sf::Color(65, 65, 65)); /*headerBg.setOutlineColor(sf::Color(80, 80, 80)); headerBg.setOutlineThickness(1);*/
        headerText.setFillColor(sf::Color::White); arrowText.setFillColor(sf::Color(180, 180, 180)); headerText.setStyle(sf::Text::Bold);headerText.setLetterSpacing(2);
    }
    void addWidget(std::shared_ptr<Widget> w) { widgets.push_back(w); }
    float getTotalHeight() const { float h = 25.0f; if (isOpen) { h += 5.0f; for (const auto& w : widgets) h += w->getHeight() + 5.0f; h += 5.0f; } return h; }
...
public:
    ...
    Sidebar(float w, const sf::Font& f) : width(w), font(f) { sbTrack.setFillColor(sf::Color(19, 19, 21)); sbThumb.setFillColor(sf::Color(80, 80, 80)); }
    ...
    void recalculateLayout() {
        ...
        for (auto& m : modules) { m->layout(x, cy); float h = m->getTotalHeight(); cy += h + 3.0f; totalContentHeight += h + 3.0f; } // MARGIN FIX
    ...
    bool isAnyWidgetHovered(const sf::RenderWindow& win) const {
        // Sprawdź czy kursor jest nad sidebarem i czy nad czymś klikalnym
        sf::Vector2f m = win.mapPixelToCoords(sf::Mouse::getPosition(win));
    ...
    void draw(sf::RenderWindow& win) {
        sf::RectangleShape bg({width, height}); bg.setPosition({x, y}); bg.setFillColor(sf::Color(19, 19, 21)); win.draw(bg);
